...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        insect.place = None
        if insect.is_ant:
            if hasattr(self.ant, 'true_queen') and self.ant.true_queen:
                insect.place = place
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if hasattr(self.ant.ant, 'true_queen') and self.ant.ant.true_queen:
...
class ThrowerAnt(Ant):
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            return target.reduce_armor(self.damage)
            ...
            while current_place:
                if current_place.ant:
                    if current_place.ant.container:
                        if current_place.ant.ant and current_place.ant.ant not in self.buffed_ants:
                            current_place.ant.ant.damage = current_place.ant.ant.damage * 2
                            self.buffed_ants.append(current_place.ant.ant)
